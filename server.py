...
async def list_tools() -> list[types.Tool]:
    return [
        types.Tool(name="odoo_search_partners", description="Search Odoo contacts/partners by name, email, or other criteria.", inputSchema={"type":"object","properties":{"query":{"type":"string"},"limit":{"type":"integer","default":10}},"required":[]}),
        types.Tool(name="odoo_get_partner", description="Get full details for a single Odoo partner/contact by ID.", inputSchema={"type":"object","properties":{"partner_id":{"type":"integer"}},"required":["partner_id"]}),
        ...
        types.Tool(name="odoo_get_record", description="Read a single Odoo record by model and ID.", inputSchema={"type":"object","properties":{"model":{"type":"string"},"record_id":{"type":"integer"},"fields":{"type":"array","default":[]}},"required":["model","record_id"]}),
        types.Tool(name="odoo_list_sale_orders", description="List sale orders.", inputSchema={"type":"object","properties":{"state":{"type":"string","default":""},"limit":{"type":"integer","default":10}},"required":[]}),
        types.Tool(name="odoo_list_invoices", description="List customer invoices.", inputSchema={"type":"object","properties":{"state":{"type":"string","default":""},"limit":{"type":"integer","default":10}},"required":[]}),
        types.Tool(name="odoo_list_crm_leads", description="List CRM leads/opportunities.", inputSchema={"type":"object","properties":{"stage":{"type":"string","default":""},"limit":{"type":"integer","default":10}},"required":[]}),
...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    try:
        if name == "odoo_search_partners":
            ...
            return ok(execute("res.partner","search_read",domain,fields=["id","name","email","phone","mobile","street","city","country_id","is_company"],limit=limit))
        elif name == "odoo_get_partner":
            records = execute("res.partner","read",[arguments["partner_id"]],fields=[])
            ...
            return ok(execute(arguments["model"],"search_read",arguments.get("domain",[]),**kwargs))
        elif name == "odoo_get_record":
            records = execute(arguments["model"],"read",[arguments["record_id"]],fields=arguments.get("fields",[]))
            return ok(records[0] if records else {})
            ...
            return ok(execute("crm.lead","search_read",domain,fields=["id","name","partner_id","stage_id","expected_revenue","probability"],limit=arguments.get("limit",10)))
